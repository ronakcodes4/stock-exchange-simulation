...
public class Order implements Tradable {
    ...
    public Order(String user, String product, Price price, int originalVolume, BookSide side)
        ...
        if (user == null || !user.matches("^[A-Za-z]{3}$")) {
            throw new InvalidOrderException("User code must be exactly 3 letters: " + user);
        }
        this.user = user;

        if (product == null || !product.matches("^[A-Za-z0-9.]{1,5}$")) {
            throw new InvalidOrderException("Product symbol must be 1-5 letters/digits or '.': " + product);
        }
        this.product = product;

        if (price == null) {
            throw new InvalidOrderException("Order price cannot be null.");
        }
        ...
        if (originalVolume <= 0 || originalVolume >= 10000) {
            throw new InvalidOrderException("Order volume must be > 0 and < 10,000: " + originalVolume);
        }
        this.originalVolume = originalVolume;
        this.remainingVolume = originalVolume;

        if (side == null) {
            throw new InvalidOrderException("Order side (BUY/SELL) cannot be null.");
        }
        this.side = side;
